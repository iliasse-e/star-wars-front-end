...
// ajoutez un événement de changement sur les checkboxes
  public onPiloteSelectionChanged(event: any, piloteId: string) {
    ...
    console.log(this.selectedPiloteIds);
  }
  ...
  public submit(): void {
    ...
    this.newMission.pilotes = this.pilotes.filter(pilote => this.selectedPiloteIds.includes(pilote.id as string));
    console.log("nom : "+ this.newMission, "ids : " + this.selectedPiloteIds)
    this.missionService.saveMission(this.newMission.nom, this.selectedPiloteIds).subscribe(mission => { // on va passer une liste d'id
  ...
  ngOnInit(): void {
    this.piloteService.getPilotesMissionReady().subscribe(pilotes => {
